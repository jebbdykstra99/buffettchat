(function () {
  ...
  function storiesComposePlaceholder() {
    return (site && site.composePlaceholder) || "What's on your mind, owner?";
  }
        ...
        showAddTray: function () {
          ...
          el.innerHTML =
            '<button type="button" class="stories-item is-add" data-story-add="1" aria-label="Add story">' +
              '<span class="stories-ring"><span class="stories-avatar" style="background:' + colorFor(SITE_ID || 'buffett') + '">' +
              escapeHtml(String(SITE_ID || 'buffett').replace(/chat$/i, '').slice(0, 3).toUpperCase() || 'ME') + '</span>' +
        ...
        setDemo: function (list, ownUid) {
          ...
          liveStories = (list || []).map(function (s, i) {
            return {
              ...
              name: s.name || (site && site.name) || 'buffettchat',
              handle: s.handle || SITE_ID || 'buffett',
              type: s.type || 'text',
              text: s.text || '',
              mediaUrl: s.mediaUrl || '',

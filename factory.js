(function () {
  ...
  let SITE_ID = 'buffettchat';
  let site = null;
  let COLORS = ['#1a2744', '#9a7d3a', '#c4a35a', '#3d4f6f', '#6b5428', '#2c3a58'];
  let TRENDS = [];
  let PLACES = [];
  let TOPICS = [];
  ...
  function applySiteChrome() {
    if (!site) return;
    var title = site.name || "buffettchat";
  ...
  function syncProfile() {
    ...
    document.getElementById('profile-bio').textContent = currentUser.bio || "The Oracle still talks.";
  ...
  fetch(SITE_JSON_URL)
    ...
    .catch(function (e) {
      ...
      boot({ siteId: "buffettchat", name: "buffettchat", tagline: "The Oracle still talks." });
    });
})();
